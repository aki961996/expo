...
def get_event_detail(event_code):
	...
	# Services — price field fetched for backend use only
	# Frontend must NOT display individual service prices
	services = []
	if t["Expo Service"]:
...
#  - At booking time: service rows saved with rate=0, amount=0
#  - Admin opens Stall Booking → edits Rate in Services table → Save
#  - stall_booking.py validate() auto-recalculates total_amount
#  - Exhibitor sees stall total only; service charges in final invoice
# ─────────────────────────────────────────────────────────────
...
def create_booking(
	...
	# ── Stall-only totals (services billed at invoice time) ───
	stall_total  = float(stall_amount or 0)
	tax_backend  = round(stall_total * 0.18)
	total_backend= stall_total + tax_backend
	deposit      = round(stall_total * 0.25)
	balance      = total_backend - deposit
	...
	booking = frappe.get_doc({
		...
		"service_amount": 0,           # filled by admin later
		"tax_amount":     tax_backend,
		"total_amount":   total_backend,
		"deposit_paid":   deposit,
		"balance_due":    balance,
	})

	# ── Add service rows with rate=0 (admin fills later) ─────
	response_services = []
	for svc in selected_services:
		...
		# Fetch display name from DB — rate stays 0 until admin sets it
		svc_display = frappe.db.get_value("Expo Service", svc_name, "service_name") or svc_name

		booking.append("services", {
			"service":      svc_name,
			"service_name": svc_display,
			"qty":          1,
			"rate":         0,     # admin will fill this
			"amount":       0,
		})
	...
	# Fallback: match by stall_number
	...
	return {
		...
		"balance_due":   balance,
		"tax_amount":    tax_backend,
		"services":      response_services,   # name only, no price
	}
